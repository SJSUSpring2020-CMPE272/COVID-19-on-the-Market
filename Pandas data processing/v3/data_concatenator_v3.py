...
def parse_from_file():
    ...
    sp500 = pd.read_csv(r'data\S&P500_index_daily_^GSPC.csv', parse_dates=[0]) #read data amd convert column 0 to datetime data type. For some docs it was parsed as a string data type otherwise
    sp500 = set_index_and_truncate(sp500)
    output_to_terminal(sp500)

    dji = pd.read_csv(r'data\DJI_index_daily_^dji.csv', parse_dates=[0])
    dji = set_index_and_truncate(dji)
    output_to_terminal(dji)

    nasdaq = pd.read_csv(r'data\Nasdaq_index_daily_^IXIC.csv', parse_dates=[0])
    nasdaq = set_index_and_truncate(nasdaq)
    output_to_terminal(nasdaq)

    w5000 = pd.read_csv(r'data\Wilshire_5000_total_market_index_^W5000.csv', parse_dates=[0])
    w5000 = set_index_and_truncate(w5000)
    output_to_terminal(w5000)

    dgs10 = pd.read_csv(r'data\10-year_treasury_constant_maturity_rate_DGS10.csv', parse_dates=[0])
    dgs10 = set_index_and_truncate(dgs10)
    output_to_terminal(dgs10)

    t10ye = pd.read_csv(r'data\10-year_breakeven_inflation_rate_T10YIE.csv', parse_dates=[0])
    ...
    infection = pd.read_csv(r'data\covid-19_daily_infection_statstics-US.csv', parse_dates=[0])
    infection = set_index_and_truncate(infection)
    output_to_terminal(infection)

    unemp = pd.read_csv(r'data\Weekly_national_initial_unemployment_claims_seasonally_adjusted.csv', parse_dates=[0])
    unemp = set_index_and_truncate(unemp)
    output_to_terminal(unemp)

    petroleum = pd.read_excel(r'data\Weekly_national_gasoline_and_diesel_prices.xlsx', parse_dates=[0])
...
df_output.to_csv(r'output\merged_data.csv')